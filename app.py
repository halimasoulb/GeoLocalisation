    ...
    def create(self, host=None, port=None, debug=None, load_dotenv=True, **options):
        ...
        def update():
            ...
            if request.method == 'POST' and form.validate():
                ...
                if len(cases) > 1 or len(cases) < 1:
                    flash('La valeur du cin est dupplique')
                else:
                    self.session.query(Case).filter(cases[0].type.value == Case.Type.NEW.value).update({Case.type : form.status.data}, synchronize_session = False)
                    self.session.commit()
                    redirect(url_for('home'))
